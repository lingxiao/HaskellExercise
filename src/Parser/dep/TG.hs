...
data TransitionArc a b = Arc { _s1 :: Tstate, _s2 :: Tstate, _input :: Input a, _output :: Output b, _prob :: Prob }
	deriving (Eq, Ord)
...
instance (Show a, Show b, Eq a, Eq b) => Show (TransitionArc a b) where
	show arc = show (arc^.s1, arc^.s2, showSym _input, showSym _output, arc^.prob)
...
composeArc a1 a2 = toArc ( min (a1^.s1) (a2^.s1), min (a1^.s2) (a2^.s2), (a1^.input), (a2^.output), (a1^.prob) * (a2^.prob))
...
relCompose g1 g2 = prune [ composeArc a1 a2 | a1 <- g1, a2 <- g2, a1^.output == a2^.input ]
